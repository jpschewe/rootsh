...
void write2syslog(const char *optr, size_t optrLength) {
  ...
  /* new length is now old buffer length (might have been 0) + current output's length */
  rptrLength += optrLength;                     
  /* tptr points to where be begin looking for crs */
  tptr = rptr;                               
  lptr = memchr(tptr, '\r', rptrLength);
  /* we found a \r at position lptr and we started searching for it at an allowed position */
